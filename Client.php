...
class Client
{
    ...
    var $_udp_timeout;            // Timeout array of the UDP connection in $timeout = array('sec'=>0,'usec'=>100000); (default 1 second) 
     ...
     * @return NULL
     *********************************************************************/
    public function __construct($ip_radius_server = '127.0.0.1', $shared_secret = '', $radius_suffix = '', $timeout = array('sec'=> 1,'usec'=> 0), $authentication_port = 1812, $accounting_port = 1813)
    {
    ...
    // Should check to see if it has ip already 
    ...
    function SetUdpTimeout($timeout = array('sec'=> 1,'usec'=> 0))
    {
	if($timeout['sec'] > 0 || $timeout['usec'] > 0)
	{
            $this->_udp_timeout = $timeout; 
        }
    }
     ...
     */
    function AccessRequest($username = '', $password = '', $udp_timeout = array('sec' => 1, 'usec' => 0), $state = NULL)
    {
    	...
    	$received_packet = chr(0); // Set this here otherwise code barfs if socket can't connect
        ...
        else
        {
            ...
            $num_sockets =  socket_select($read_socket_array, $write_socket_array, $except_socket_array, $this->_udp_timeout['sec'], $this->_udp_timeout['usec']);

	    if($num_sockets === FALSE)
	    {
		$this->_last_error_code    = socket_last_error();
		$this->_last_error_message = "Possible SELECT TIMEOUT socket returned false " . socket_strerror($this->_last_error_code);
	    ...
	    {
		$this->_last_error_code    = socket_last_error();
		$this->_last_error_message = "Possible SELECT TIMEOUT 0 Sockets returned " . socket_strerror($this->_last_error_code);
                socket_close($_socket_to_server);
		return -1;
            }
	    else
	    {
		// We should have sockets returned but lets check to make sure
		...
		{
                        $this->_last_error_code    = socket_last_error();
			$this->_last_error_message = "Socket not found in read array " . socket_strerror($this->_last_error_code);
			return -1;
		}
            }
        }
	...
	elseif($this->_radius_packet_received == 3) // Access-Reject
	{
		return 0;
	}		
	else
